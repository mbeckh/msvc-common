...
exports.coverage = async function() {
  try {
    ...
    await exec.exec('bash', [ '-c', `./${codacyScript} report -r '${path.posix.join(solutionPath, 'bin', '*_coverage.xml')}' -t ${codacyToken} --commit-uuid ${env.GITHUB_SHA}` ]);
...
exports.analyzeReport = async function() {
  try {
    ...
    core.info(toolPath);
    core.info(path.posix.normalize(toolPath));
