...
exports.analyzeReport = async function() {
  try {
    ...
    await exec.exec('bash', [ '-c', `find ${path.posix.join(tempPath, 'clang-tidy', path.posix.sep)} -maxdepth 1 -name '*.log' -exec cat {} \\; | java -jar ${toolPath.replace(/\\/, '/')} | sed -r -e "s#[\\\\]{2}#/#g" > ${logFile}` ]);
