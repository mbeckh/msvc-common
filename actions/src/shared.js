...
exports.coverage = async function() {
  try {
    ...
    await exec.exec('bash', [ '-c', `cat ${path.posix.join(path.posix.forceNormalize(solutionPath), 'bin', '*_coverage.xml')} | sed -r -e "s#>D:<#>.<#g" -e "s#D:[\\\\]a[\\\\]llamalog[\\\\]llamalog[\\\\]##g" -e "s#a[\\\\]llamalog[\\\\]llamalog[\\\\]##g" -e "s#[\\\\]#/#g" > bin/cov.xml` ]);
    await exec.exec('bash', [ '-c', `./${codacyScript} report -r '${path.posix.join(path.posix.forceNormalize(solutionPath), 'bin', 'cov.xml')}' -t ${codacyToken} --commit-uuid ${env.GITHUB_SHA}` ]);
